...
def variable_init(size):
    ...
    return tf.random_normal(shape=size, stddev=w_stddev) #Outputs random values from a normal distribution


# 定义输入矩阵的占位符，输入层单元为784，None代表批量大小的占位，X代表输入的真实图片。占位符的数值类型为32位浮点型 784 = 28*28
...
# src=cv2.imread('F://20.jpg',flags=0)
# Mat = cv2.resize(src,(100, 100), interpolation=cv2.INTER_CUBIC)
# Mat = Mat.reshape((100,100))
# cv2.namedWindow('input_image', cv2.WINDOW_AUTOSIZE)
# cv2.imshow('input_image', Mat)
# cv2.waitKey(0)
# cv2.destroyAllWindows()

# Mat = cv2.imread('F://20.jpg')
# Mat = cv2.resize(Mat,)
# cv2.imshow(Mat)
...
def sample_Z(m, n):
    # src=cv2.imread('F://20.jpg',flags=0)
#     # Mat = cv2.resize(src,(m, n), interpolation=cv2.INTER_CUBIC)
#     # return (Mat.reshape((m,n))/255 - 1)
...
def generator(z):
    ...
    G_prob = tf.nn.sigmoid(G_log_prob) #用sigmoid将图像进行压缩 输出信息(?, 784)
    return G_prob
...
# 输入随机噪声z而输出生成样本
#再采样得到的噪声数据更新生成器的参数 每次生成的参数*初始输入的样本 = 生成的图片
G_sample = generator(Z) #G(Z)

# 分别输入真实图片和生成的图片，并投入判别器以判断真伪
D_real, D_logit_real = discriminator(X) #D(X)
D_fake, D_logit_fake = discriminator(G_sample) #fake假的  logit逻辑回归 D(G(Z))
...
#labels=tf.zeros_like(D_logit_fake) 创建维度和D_logit_real相等的全是0的标注，生成图片
...
# 同样使用交叉熵构建生成器损失函数
"""
因为如果只使用生成网络，那么无法得到误差，也就无法训练
此时是通过判别器来生成误差的，而误差回传的目的是使得生成器生成的假样本逐渐逼近为真样本
(当假样本不真实，标签却为1时，判别器给出的误差会很大，这就迫使生成器进行很大的调整
"""
...
mb_size = 128
Z_dim = 100
# cv2.imread("D://deep_learn_cond//GAN1//save//2.jpg",sample_Z(16, Z_dim))
